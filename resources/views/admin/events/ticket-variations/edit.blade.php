<x-admin.app-layout>
    ...
    <h4 class="fw-bold py-3 mb-3 d-flex justify-content-between align-items-center">
        ...
        <a href="{{ url("dashboard/events/$event->slug") }}" type="button" class="btn btn-sm btn-secondary">Kembali</a>
    </h4>

    <form action="{{ url("dashboard/events/$event->slug/ticket/$ticket->id") }}" method="POST" data-parsley-validate>
        @csrf
        @method('PATCH')
        ...
        <div id="form-container">
            <div class="card mb-3">
                ...
                <div class="card-body card-ticket-1">
                    <div class="mb-3">
                        <x-admin.form-input label="Name" name="name" value="{{ $ticket->name }}" />
                    </div>
                    <div class="mb-3">
                        <div class="row">
                            <div class="col">
                                <x-admin.form-input label="Price" name="price" type="number"
                                    information="Input 0 to set free ticket" value="{{ $ticket->price }}" />
                            </div>
                            <div class="col">
                                <x-admin.form-input label="Quota" name="quota" type="number" value="{{ $ticket->quota }}" />
                            </div>
                            <div class="col">
                                <x-admin.form-input label="Maximum Purchase User" name="max_user_purchase"
                                    type="number" value="{{ $ticket->max_user_purchase }}" />
                            </div>
                        </div>
                    </div>
                    <div class="mb-3">
                        <x-admin.form-textarea label="Description" name="description" value="{{ $ticket->description }}" />
                    </div>
                </div>
            </div>
        </div>
